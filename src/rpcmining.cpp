...
Value getgenerate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or startupcoin.conf setting gen)\n"
...
Value setgenerate(const Array& params, bool fHelp)
{
    ...
    if (fGenerate && Params().NetworkID() == CChainParams::REGTEST)
    {
        ...
        while (nHeight < nHeightEnd)
        {
            ...
            {
                nHeightLast = nHeight;
                GenerateStartUpcoins(fGenerate, pwalletMain, 1);
            }
            MilliSleep(1);
            {   // Don't keep cs_main locked
                LOCK(cs_main);
    ...
    else // Not -regtest: start generate thread, return immediately
    {
        ...
        GenerateStartUpcoins(fGenerate, pwalletMain, nGenProcLimit);
    }
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "See https://en.startupcoin.it/wiki/BIP_0022 for full specification.\n"
