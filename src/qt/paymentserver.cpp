// Copyright (c) 2011-2013 The StartUpcoin developers
...
const int STARTUPCOIN_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString STARTUPCOIN_IPC_PREFIX("startupcoin:");
const char* STARTUPCOIN_REQUEST_MIMETYPE = "application/startupcoin-paymentrequest";
const char* STARTUPCOIN_PAYMENTACK_MIMETYPE = "application/startupcoin-paymentack";
const char* STARTUPCOIN_PAYMENTACK_CONTENTTYPE = "application/startupcoin-payment";
...
bool PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(STARTUPCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // startupcoin: URI
        {
            savedPaymentRequests.append(arg);

            SendCoinsRecipient r;
            if (GUIUtil::parseStartUpcoinURI(arg, &r))
            {
                CStartUpcoinAddress address(r.address.toStdString());
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    foreach (const QString& r, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(STARTUPCOIN_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            return false;
        }
        ...
        socket->waitForBytesWritten(STARTUPCOIN_IPC_CONNECT_TIMEOUT);
    ...
    optionsModel(0)
{
    ...
    // on Mac: sent when you click startupcoin: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start startupcoin: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling startupcoin: URIs and
// PaymentRequest mime types
//
bool PaymentServer::eventFilter(QObject *object, QEvent *event)
{
    // clicking on startupcoin: URIs creates FileOpen events on the Mac
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in startupcoin: URIs
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(STARTUPCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // startupcoin: URI
    {
#if QT_VERSION < 0x050000
        QUrl uri(s);
#else
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseStartUpcoinURI(s, &recipient))
                emit receivedPaymentRequest(recipient);
            else
                emit message(tr("URI handling"),
                    tr("URI can not be parsed! This can be caused by an invalid StartUpcoin address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    foreach(const PAIRTYPE(CScript, qint64)& sendingTo, sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            addresses.append(QString::fromStdString(CStartUpcoinAddress(dest).ToString()));
        }
        else if (!recipient.authenticatedMerchant.isEmpty()){
            // Insecure payments to custom startupcoin addresses are not supported
        ...
        if (txOut.IsDust(CTransaction::nMinRelayTxFee)) {
            QString msg = tr("Requested payment amount of %1 is too small (considered dust).")
                .arg(StartUpcoinUnits::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second));
...
void PaymentServer::fetchRequest(const QUrl& url)
{
    ...
    netRequest.setRawHeader("Accept", STARTUPCOIN_REQUEST_MIMETYPE);
...
void PaymentServer::fetchPaymentACK(CWallet* wallet, SendCoinsRecipient recipient, QByteArray transaction)
{
    ...
    netRequest.setHeader(QNetworkRequest::ContentTypeHeader, STARTUPCOIN_PAYMENTACK_CONTENTTYPE);
    netRequest.setRawHeader("User-Agent", CLIENT_NAME.c_str());
    netRequest.setRawHeader("Accept", STARTUPCOIN_PAYMENTACK_MIMETYPE);
